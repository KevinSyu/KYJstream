...
class FrameWork:
  ...
  def init():
    ...
    app.config['JWT_ACCESS_TOKEN_EXPIRES'] = datetime.timedelta(minutes=KYJStreamConfig.get_int(FrameWork.jwt_config_section,'ACCESS_TOKEN_EXPIRES_TIME'))
    app.config['JWT_REFRESH_TOKEN_EXPIRES'] = datetime.timedelta(days=KYJStreamConfig.get_int(FrameWork.jwt_config_section,'REFRESH_TOKEN_EXPIRES_TIME'))
    jwt.init_app(app)
